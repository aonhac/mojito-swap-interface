...
import { airdrop } from 'constants/airdrop'
...
function fetchClaim(account: string, chainId: ChainId): any {
  const formatted = isAddress(account)
  if (!formatted) return new Error('Invalid address')
...
export function useUserClaimData(account: string | null | undefined): UserClaimData | null | undefined {
  ...
  useEffect(() => {
    ...
    const accountClaimInfo = fetchClaim(account, chainId)
...
export function useClaimed(library: any): boolean {
  ...
  return claimed.result?.[0] === true
}

// check if user is in blob and has not yet claimed UNI
export function useUserHasAvailableClaim(library: any): boolean {
  const { account } = useWeb3React()
  const userClaimData = useUserClaimData(account)
  const distributorContract = getAirdropContract(library)
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unclaimedAmount?.toSignificant(4)} MJT`,
