...
import { TokenAmount, Token, ChainId, Percent, JSBI } from 'mojito-testnet-sdk'
...
describe('utils', () => {
  describe('#getKCCExplorerLink', () => {
    it('correct for tx', () => {
      expect(getKCCExplorerLink(1, 'abc', 'transaction')).toEqual('https://bscscan.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getKCCExplorerLink(1, 'abc', 'token')).toEqual('https://bscscan.com/token/abc')
    })
    it('correct for address', () => {
      expect(getKCCExplorerLink(1, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getKCCExplorerLink(2, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
    it('ropsten', () => {
      expect(getKCCExplorerLink(3, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
    it('enum', () => {
      expect(getKCCExplorerLink(ChainId.MAINNET, 'abc', 'address')).toEqual('https://bscscan.com/address/abc')
    })
  })
