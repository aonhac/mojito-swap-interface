...
const RecentTransactionsModal = ({ onDismiss = defaultOnDismiss, translateString }: RecentTransactionsModalProps) => {
  ...
  return (
    <Modal title={translateString(1202, 'Recent transactions')} onDismiss={onDismiss} style={{ width: '550px' }}>
      {!account && (
          ...
          return (
            <>
              <Flex key={hash} alignItems="center" justifyContent="space-between" mb="4px">
                <LinkExternal href={getKCCExplorerLink(chainId, hash, 'transaction')} color={color}>
                  {summary ?? hash}
                </LinkExternal>
                {icon}
              </Flex>
