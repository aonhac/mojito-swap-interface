...
const BarItem: React.FunctionComponent<{
  pair: Pair
  row: number
}> = ({ pair, row }) => {
  ...
  return (
    <BarItemWarp index={row}>
      <RowFixed>
        ...
        <AutoColumn>
          <PrimaryTitleMD style={{ color: theme.colors.text }}>
            {currency0.symbol}-{currency1.symbol}
          </PrimaryTitleMD>
          <GreyTitle style={{ fontSize: '14px', marginTop: '3px' }}>
            Liquidity {totalPoolTokens?.toSignificant(4)}
          </GreyTitle>
        </AutoColumn>
      </RowFixed>
      <RowBetween style={{ marginTop: '18px', alignItems: 'flex-end' }}>
        <RowFlat>
          <PrimaryTitleMD style={{ color: theme.colors.text, fontSize: '16px', marginRight: '8px' }}>
            APR
          </PrimaryTitleMD>
          <Text fontSize="28px" fontWeight="800" color="#FF7B1B" lineHeight="1">
            300.00 %
          </Text>
        </RowFlat>
