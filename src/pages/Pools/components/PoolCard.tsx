...
const PoolCard: React.FC<HarvestProps> = ({ pool }) => {
  ...
  // APR
  const rewardTokenPrice: any = useGetApiPrice(earningToken.address ? getAddress(earningToken.address) : '')
  const stakingTokenPrice: any = useGetApiPrice(stakingToken.address ? getAddress(stakingToken.address) : '')
  const apr = getPoolApr(
    stakingTokenPrice,
    rewardTokenPrice,
    getBalanceNumber(pool?.totalStaked as any, stakingToken.decimals),
                ...
                <Button
                  ...
                  onClick={
                    isOldSyrup
                      ? async () => {
                          setPendingTx(true)
                          await onUnstake('0', stakingToken?.decimals as any)
                          setPendingTx(false)
                        }
                      : onPresentWithdraw
                  }
