...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({ details, apr, multiplier, liquidity }) => {
  ...
  const bsc = `${process.env.REACT_APP_EXPLORER_URL}/address/${lpAddress}`
  const info = `${process.env.REACT_APP_INFO_URL}/pair/${lpAddress}`
  ...
  return (
    <Container>
      <InfoContainer>
        {isActive && (
          <StakeContainer>
            <StyledLinkExternal href={`${process.env.REACT_APP_OFFICIAL_URL}/#/add/${liquidityUrlPathParts}`}>
